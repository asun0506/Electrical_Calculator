...
(async () => {
  ...
  assert.deepEqual(counts, ['46', '115']);
  ...
  assert.equal(await page.locator('#dfWorkBody tr').count(), 161);
  assert.match(await page.locator('#dfWorkCount').innerText(), /共 161 行/);
  ...
  assert.ok(sorted.slice(46).every((row) => row.c === '电气系统'), 'integrated component rows should be second');
  const componentNames = sorted.slice(46).map((row) => row.d);
  ...
  [sorted.slice(0, 46), sorted.slice(46)].forEach((levelRows) => {
  ...
  await browser.close();
  console.log('PASS DFMEA browser two-level hierarchy and 161-row export');
