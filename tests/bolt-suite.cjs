...
(async()=>{
  ...
  await page.locator('[data-image-kind="part"][data-image-owner]').first().setInputFiles({name:'EDM.png',mimeType:'image/png',buffer:png});await page.locator('.bt-image img').first().waitFor();assert.match(await page.locator('.bt-image .bt-file').first().innerText(),/更换附图/);await page.locator('.bt-image-preview').first().click();assert.equal(await page.locator('#engineering-image-preview').evaluate(el=>el.style.display),'flex');await page.locator('#engineering-image-preview button').click();
