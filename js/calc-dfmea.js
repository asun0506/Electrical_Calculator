...
(function () {
  ...
  function sortWorkRows(){
    const collator=new Intl.Collator('zh-CN',{numeric:true,sensitivity:'base'});
    const text=(row,field)=>String(row[field]||'');
    state.rows=state.rows.map((item,index)=>({item,index})).sort((a,b)=>{
      const levelA=Number(a.item.level)||99,levelB=Number(b.item.level)||99;
      if(levelA!==levelB)return levelA-levelB;
      const fields=['D','E','G','K','C'];
