...
(function () {
  ...
  function cellEditor(table, row, col, header, value) {
    ...
    return `<div class="sor-image-cell">${textarea}
      <div class="sor-image-control">${image ? `<button type="button" class="sor-image-preview" data-preview-cell-image="${esc(key)}" title="单击放大附图"><img src="${esc(image.dataUrl)}" alt="${esc(image.name)}"></button><label class="btn sor-file-btn">更换附图<input type="file" accept="image/png,image/jpeg,.png,.jpg,.jpeg" data-cell-image="${esc(key)}"></label><button type="button" data-remove-cell-image="${esc(key)}">移除图片</button>` : `<label class="btn sor-file-btn">插入图片<input type="file" accept="image/png,image/jpeg,.png,.jpg,.jpeg" data-cell-image="${esc(key)}"></label><small>图片将插入 Word/PDF 中的此处</small>`}</div>
    </div>`;
  }
  ...
  function bindEvents() {
    ...
    hostRef.querySelectorAll('[data-preview-cell-image]').forEach((button) => button.addEventListener('click', () => {
      const image = state.cellImages?.[button.dataset.previewCellImage];
      if (image) E.previewImage(image.dataUrl, image.name || '报告附图');
    }));
