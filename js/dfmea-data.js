...
(function (global) {
  ...
  function mergeLevelControl(parentValue,childValue,prefix){
    const parentText=String(parentValue||'').trim();
    const childText=String(childValue||'').replace(prefix,'').trim();
    if(!parentText)return childText;if(!childText||childText===parentText)return parentText;
    return `部件级：${parentText}\n子零件级：${childText}`;
  }
  const integratedL2=[];
  l2.forEach((parent)=>{
    const linked=l3.filter((child)=>child.C===parent.D&&child.F===parent.G&&child.I===parent.K);
    const exact=parent.E?linked.filter((child)=>child.D===parent.E&&child.G===parent.H&&child.K===parent.L):[];
    const children=exact.length?exact:linked;
    ...
    children.forEach((child,index)=>integratedL2.push({
      ...
      H:child.G,
      L:child.K,
      M:mergeLevelControl(parent.M,child.M,/^子零件控制：/),
      N:child.N||parent.N,
      O:mergeLevelControl(parent.O,child.O,/^子零件验证：/),
      P:child.P||parent.P,
  ...
  // 子零件专项扩展：系统级父功能 -> 二级部件功能/失效 -> E/H/L子零件特性/原因。
  const extraSystemSpecs={
    switching:['高压开关器件在全部工况下可靠接通、承载并分断主回路','高压主回路异常断开、无法隔离或局部过热',10,'开关器件额定值、负载谱、驱动、灭弧与诊断架构评审',3,'边界电压、温升、分断、振动及故障注入试验',3],
    precharge:['预充回路按规定时序、能量和寿命要求建立母线电压','预充失败、超时、浪涌过大或回路过热',9,'预充RC、脉冲能量、器件降额、散热和诊断策略评审',4,'全温全压预充波形、连续循环、寿命及故障注入试验',3],
    sensing:['电流测量链在全量程、全温度和电磁环境下准确且可诊断','电流测量错误导致保护、功率或SOC控制异常',9,'量程、带宽、温漂、磁路、采样和诊断覆盖评审',3,'全温标定、过载、EMC、响应时间及故障注入试验',4],
    distribution:['包内高压导体和连接界面低阻、耐久并承受故障电流','包内高压回路开路、短路、压降或温升超限',10,'电流map、截面积、连接、短路电动力、热与机械耐久设计',3,'四线电阻、温升、短路耐受、振动和环境试验',3],
    signal:['采样与控制电路在环境及EMC条件下连续、准确并相互隔离','采样或控制信号错误、丢失或通道间短路',9,'电路误差预算、布线隔离、焊点、连接器和诊断覆盖评审',3,'开短路注入、全温精度、EMC、振动和湿热试验',4],
    protection:['故障电流切断器件按规定时间安全动作且正常工况不误动作','故障电流未切断、误切断或分断过程产生危险',10,'保护配合、I²t、分断容量、触发回路、灭弧及防错设计',3,'时间电流、最大短路分断、误动作及环境耐久试验',3],
    lvinterface:['低压供电和信号连接在寿命及环境条件下连续、低阻且防错','低压供电或控制信号中断、短路、错接或过热',8,'接口矩阵、线径、压接、锁止、密封、路由和防错评审',4,'导通、压降、温升、插拔、振动、密封及故障注入试验',4],
    grounding:['低压接地及等电位连接在寿命和故障工况下保持低阻连续','接地或等电位连接高阻、中断或过热',10,'接地拓扑、截面积、压接、表面处理、防松和防腐设计',3,'四线电阻、故障电流、扭矩保持、振动和盐雾试验',3],
    thermal:['温度测量链准确、快速且可诊断地反映冷却系统温度','温度测量错误导致热保护或热管理控制异常',9,'量程、特性曲线、响应、安装热阻、密封和诊断覆盖评审',3,'全温标定、响应、开短路、振动、密封和环境试验',4],
    hvconnector:['高压连接器在插合寿命及环境条件下低阻、密封、防错并防触电','高压接口开路、短路、过热、进水或触电防护失效',10,'额定值、端子、锁止、HVIL、密封、屏蔽、防错和防触电设计',3,'温升、动态电阻、插拔、振动、IP、EMC及探针试验',3],
    hvcable:['高压线缆在电流、机械、热及化学环境下保持导通、绝缘和屏蔽','高压线缆开路、短路、过热、绝缘或屏蔽失效',10,'线径、绝缘、屏蔽、压接、弯曲、路由、耐磨和环境降额设计',3,'温升、压降、弯折、振动、耐磨、化学和耐压试验',3],
    hvil:['高压互锁回路对连接状态的检测连续、可信且抗干扰','互锁状态误判导致带电操作或非预期高压切断',10,'HVIL拓扑、端子时序、线路防护、诊断阈值和抗扰设计',3,'开短路、高阻、插拔时序、振动及EMC故障注入试验',3],
  };
  const extraParents={};
  Object.entries(extraSystemSpecs).forEach(([key,spec],index)=>{
    const parent=row(1,`SYS-EX-${String(index+1).padStart(2,'0')}`,'','电气系统','','',spec[0],'','',spec[2],spec[1],'',spec[3],spec[4],spec[5],spec[6],['电气系统','子零件专项扩展']);
    system.push(parent);extraParents[key]=parent;
  });
  const controlTemplates={
    contact:['材料镀层、接触正压力、额定电流和连接界面设计','四线法接触电阻、温升及振动后动态电阻测试'],
    timing:['驱动裕量、动作时间、公差和控制时序预算','全温全压动作时间、波形和时序边界测试'],
    feedback:['反馈触点、信号合理性和状态交叉诊断设计','状态切换、卡滞及不一致故障注入测试'],
    electrical:['电压电流裕量、回路保护和开短路诊断设计','边界电压、开短路和功耗温升测试'],
    arc:['分断容量、触点间距、灭弧和绝缘恢复设计','最大电流分断、燃弧时间和分断后耐压测试'],
    vibration:['固定、应变释放、模态和抗微动磨损设计','随机振动、机械冲击和动态导通监测'],
    thermal:['功率/能量降额、热阻、材料耐温和散热路径设计','最大负载温升、热冲击和热循环后复测'],
    resistance:['材料、几何、温度系数和阻值公差设计','全温阻值、脉冲负载和寿命后漂移测试'],
    mechanical:['强度、公差、固定、限位和装配工艺设计','尺寸、保持力、耐久、振动和极限工况测试'],
    emc:['屏蔽、滤波、接地、布线隔离和抗扰度设计','BCI/辐射抗扰、瞬态及监测信号测试'],
    sensing:['量程、精度、响应、温漂和安装位置误差预算','全温标定、动态响应、线性度和交叉验证'],
    sealing:['密封材料、压缩率、界面公差和装配防错设计','气密、浸水、IP及热循环后密封测试'],
    short:['绝缘间距、线路保护、异物和装配防错设计','线间/对地短路注入、耐压和截面检查'],
    durability:['负载谱、循环次数、材料疲劳和寿命降额设计','全寿命循环及关键参数趋势监测'],
    corrosion:['材料相容性、镀层、密封和电化学防腐设计','盐雾、湿热、化学介质和环境后电阻测试'],
    crimp:['压接高度、截面、拉脱力、材料及工艺窗口设计','压接截面、拉脱力、毫伏压降和过程能力验证'],
    routing:['最小弯曲半径、固定点、锐边和运动干涉防护设计','布置检查、弯折、耐磨、振动和动态导通测试'],
    shield:['屏蔽覆盖率、360°端接和接地拓扑设计','屏蔽连续性、转移阻抗和EMC测试'],
    diagnostic:['故障阈值、合理性、回路监控和降级策略设计','开短路、卡滞、高阻和边界故障注入测试'],
    assembly:['接口唯一化、零件标识、工装限位和装配防错设计','错装、漏装、极限尺寸和装配过程审核'],
    safety:['安全间距、能量约束、材料阻燃和故障包容设计','滥用、过载、热失控传播及安全边界验证'],
  };
  const extraCatalog=[
    ['EDM（电源分配单元）','主继电器','switching',[
      ['闭合后以规定接触电阻持续承载主回路电流','接触电阻增大导致局部过热','contact',4,3],['吸合过程中保持触点稳定接触','触点弹跳导致母线电压波动或电弧','timing',3,4],['断电后在规定时间内释放触点','释放延迟','timing',3,3],['通过辅助触点反馈主触点真实状态','辅助触点反馈卡滞或状态错误','feedback',3,4],['线圈在工作电压范围内保持可靠励磁','线圈开路或匝间短路','electrical',3,3],['分断后建立足够触点间绝缘距离','触点间燃弧重击穿','arc',2,4],['在振动和冲击环境下保持触点状态','振动导致触点抖动或瞬时断开','vibration',3,4]]],
    ['EDM（电源分配单元）','预充继电器','precharge',[
      ['闭合后以低接触电阻承载预充电流','接触电阻增大','contact',4,3],['预充完成后及时断开预充支路','释放延迟','timing',3,3],['吸合时避免异常触点弹跳','触点弹跳导致预充电流重复冲击','timing',3,4],['反馈预充继电器真实开闭状态','辅助触点反馈与主触点状态不一致','feedback',3,4],['承受重复预充产生的触点电弧能量','触点烧蚀导致寿命提前终止','durability',4,4],['在振动环境中维持稳定状态','振动导致瞬时误断开','vibration',3,4]]],
    ['EDM（电源分配单元）','预充电阻','precharge',[
      ['在异常控制条件下避免形成低阻旁路','内部短路','short',2,4],['承受单次最大预充脉冲能量','电阻体受热开裂','thermal',4,4],['通过安装界面有效散热','与散热面的接触热阻过大','thermal',4,3],['在温度范围内保持规定阻值','温度系数异常导致阻值随温度漂移','resistance',3,4],['保持引脚、焊点或螺栓连接可靠','端部连接松动或焊点疲劳开裂','vibration',3,4],['故障过热时不引燃周边材料','过热引起封装破裂或周边材料燃烧','safety',2,4]]],
    ['EDM（电源分配单元）','霍尔传感器','sensing',[
      ['按正确方向输出充放电电流极性','安装方向或极性错误','assembly',2,2],['在规定响应时间内跟踪电流变化','响应延迟','sensing',3,4],['在全温度范围内保持测量精度','温度漂移导致增益误差超限','sensing',4,4],['在供电异常时输出可诊断状态','供电开路或欠压','diagnostic',3,3],['在电磁干扰下保持输出稳定','外部磁场或EMC干扰导致信号跳变','emc',4,5],['保持传感器与导体的设计位置','安装偏移或气隙变化导致比例误差','mechanical',3,4],['支持开路、短路和卡滞诊断','输出卡滞但未被诊断识别','diagnostic',3,5]]],
    ['EDM（电源分配单元）','EDM铜排','distribution',[
      ['通过连接界面低阻传输主回路电流','连接面接触电阻增大','contact',4,3],['承受短路电流产生的电动力','短路电动力作用下变形或移位','mechanical',2,4],['承受振动及热循环载荷','根部或折弯处疲劳开裂','vibration',3,4],['保持表面镀层和连接面耐腐蚀','镀层破损或氧化导致接触电阻升高','corrosion',3,4],['保持螺栓连接预紧力','紧固件松动导致局部过热','mechanical',3,3],['避免装配偏差造成邻近件干涉','尺寸或位置偏差导致短路风险','assembly',2,3]]],
    ['EDM（电源分配单元）','转接PCB','signal',[
      ['保持高低压网络之间的电气间隙和爬电距离','表面污染或凝露导致爬电','short',3,4],['保持焊点在振动及热循环下可靠','焊点疲劳开裂导致间歇开路','vibration',4,5],['防止相邻采样通道短路','铜箔、焊锡或异物造成通道间短路','short',3,3],['正确传输采样信号并抑制串扰','布线串扰导致采样偏差','emc',3,5],['在潮湿环境下防止电化学迁移','发生CAF或离子迁移','corrosion',3,5],['保持连接器和PCB机械固定','连接器焊盘或固定点开裂','mechanical',3,4],['在EMC环境下保持采样信号稳定','瞬态干扰耦合进入采样通道','emc',4,5]]],
    ['保险丝盒','Shunt（电流传感器）','sensing',[
      ['在全温度范围内保持规定电阻值','温度系数导致测量误差','resistance',4,4],['承受短时过载及故障电流','过载后永久阻值漂移','thermal',3,4],['保持主体机械完整','电阻体或焊缝疲劳开裂','vibration',3,4],['保持Kelvin采样端极性正确','Kelvin采样端接反','assembly',2,2],['避免主电流连接压降进入采样回路','Kelvin取样位置错误导致附加误差','assembly',3,3],['在热梯度条件下保持测量精度','热电势引起零点偏移','sensing',3,5],['维持低阻主电流连接','主端连接松动并局部过热','contact',3,3]]],
    ['保险丝盒','Pyro-fuse','protection',[
      ['在规定时间内完成触发和切断','触发延迟','timing',2,4],['触发后建立足够的永久隔离距离','切断后燃弧重击穿','arc',2,4],['触发前持续承载正常工作电流','内部连接电阻增大','contact',3,3],['通过点火回路反馈可诊断电阻','点火器回路电阻异常但未被诊断','diagnostic',3,4],['在振动和温度循环后保持点火能力','点火药剂或点火连接老化失效','durability',2,5],['防止错误电信号触发','EMC或线路短路导致非预期触发','emc',2,4],['触发后约束碎片和高温气体','碎片或气体损伤邻近零件','safety',2,4]]],
    ['保险丝盒','辅助回路保险丝','protection',[
      ['在额定故障电流下安全灭弧','分断能力不足导致持续燃弧','arc',2,4],['通过端子保持低阻连接','端子接触电阻增大','contact',4,3],['在振动环境中保持可靠夹持','保险丝松动造成间歇断路','vibration',3,4],['在温度循环后保持时间—电流特性','熔体老化导致熔断特性漂移','durability',3,5],['防止装入错误额定值的保险丝','保险丝规格错装','assembly',2,2],['熔断时不造成外壳破裂或喷弧','故障能量超过分断能力','safety',2,4]]],
    ['保险丝盒','保险丝盒铜排','distribution',[
      ['通过保险丝连接界面保持低接触电阻','保险丝座连接面接触电阻增大','contact',4,3],['在多支路间按设计分配电流','支路截面积或连接差异导致电流分配异常','thermal',3,4],['承受短路电动力','铜排变形后与邻近导体接触','mechanical',2,4],['承受振动和热循环','焊接或铆接位置疲劳开裂','vibration',3,4],['保持表面镀层完整','镀层腐蚀导致连接点温升','corrosion',3,4],['防止异物跨接不同电位铜排','金属异物造成铜排间短路','short',2,3]]],
    ['低压线束','低压连接器','lvinterface',[
      ['通过CPA/TPA结构保持连接器和端子锁止','CPA/TPA未安装到位','assembly',3,2],['在振动条件下保持稳定接触','微动磨损导致间歇性高电阻','vibration',4,4],['密封接口防止水和污染物进入','密封圈缺失、错装或压缩不足','sealing',3,3],['防止相邻针脚发生短路','端子变形或异物造成针脚间短路','short',3,3],['在额定电流下限制端子温升','端子接触电阻过大导致过热','contact',4,3],['保持规定插拔寿命','多次插拔后端子正压力不足','durability',3,4],['防止端子与线缆压接处进水','线封失效导致毛细进水','sealing',3,4]]],
    ['低压线束','线缆','lvinterface',[
      ['保持导体与车身地绝缘','线缆磨损导致对地短路','routing',4,3],['保持不同信号导体相互绝缘','线间短路导致错误控制信号','short',3,3],['在弯曲和振动条件下保持导体连续','铜丝疲劳断裂导致间歇开路','vibration',4,5],['满足规定最小弯曲半径','布线弯曲半径不足导致绝缘层开裂','routing',3,3],['承受周边最高环境温度','热老化导致绝缘脆化','thermal',3,5],['保持通信线的绞距和阻抗','解绞长度过大或阻抗不匹配','emc',3,4],['防止与锐边及运动件摩擦','布置错误导致磨损','routing',4,3],['阻止液体沿线缆进入连接器','液体沿导体毛细渗透','sealing',3,5]]],
    ['低压线束','低压接线端子','lvinterface',[
      ['通过压接形成规定的电气和机械连接','压接高度过大导致接触电阻增加','crimp',4,3],['通过压接保持规定拉脱力','压接高度过小或压伤导体导致拉脱力不足','crimp',3,3],['完整包覆导体且不夹入绝缘层','绝缘层被压入导体压接区','crimp',3,3],['保持端子倒刺和锁止结构完整','端子保持力不足导致退针','mechanical',3,3],['防止铜线与端子材料电化学腐蚀','异种金属或进水导致压接区腐蚀','corrosion',3,5],['保持线封区域完整','端子压接损伤线封导致进水','sealing',3,4]]],
    ['低压线束','低压OT端子','grounding',[
      ['以规定扭矩形成低阻接地连接','紧固扭矩不足导致搭铁电阻升高','mechanical',4,3],['防止螺栓连接在振动中松动','防松结构失效','vibration',3,3],['保持OT端子与线缆的压接强度','压接区导体松脱或断股','crimp',3,3],['保持接地连接面耐腐蚀','接地面氧化或电化学腐蚀','corrosion',4,4],['防止端子随螺栓转动','OT端子未限位并发生旋转','assembly',3,3],['承受接地故障电流','OT端子截面积不足导致过热','thermal',2,4]]],
    ['低压线束','水温传感器','thermal',[
      ['在规定时间内响应冷却液温度变化','传感器热响应过慢','sensing',3,4],['保持温阻特性与软件标定匹配','传感器特性曲线型号错装','assembly',2,2],['正确接触被测介质或测温表面','安装位置或插入深度错误','mechanical',3,3],['保持信号不对电源或地短路','温度信号对电源或地短路','short',3,3],['防止冷却液从安装界面泄漏','密封结构失效导致冷却液泄漏','sealing',3,3],['在振动条件下维持稳定输出','内部连接间歇开路','vibration',3,5],['支持超量程及线路故障诊断','故障输出处于合理范围而未被识别','diagnostic',3,5]]],
    ['高压线束','高压连接器','hvconnector',[
      ['在额定RMS电流下限制端子温升','端子接触电阻增大导致过热','contact',4,3],['保持端子轴向保持力','端子保持结构失效导致退针','mechanical',3,3],['通过CPA保持连接器完全锁止','CPA未到位但连接器可通电','assembly',3,3],['保持屏蔽层360°连续连接','屏蔽端接不连续导致EMC性能下降','shield',3,4],['防止正负高压端子间短路','端子变形、异物或冷凝水造成极间短路','short',2,3],['在振动环境下避免瞬时断开','端子微动磨损导致间歇开路','vibration',4,5],['在多次插拔后保持接触正压力','插拔磨损导致接触电阻升高','durability',3,4],['未插合时保护密封和端子界面','防尘盖缺失导致端子污染','sealing',3,3],['防止错型号连接器相互插合','不同回路连接器防错设计不足','assembly',2,2],['在高海拔和污染环境下防止表面放电','污染或凝露导致端子间爬电','short',2,4],['连接器拔出前完成HVIL断开','HVIL短针尺寸错误导致晚断功能失效','timing',3,3],['在碰撞载荷下维持必要状态','壳体或锁止结构碰撞破裂','mechanical',2,4]]],
    ['高压线束','高压线缆','hvcable',[
      ['保持高压导体连续','导体断股或疲劳断裂造成间歇开路','vibration',4,5],['保持导体与端子的机械连接','线缆从压接端子中拉脱','crimp',3,3],['保持屏蔽层连续和规定转移阻抗','屏蔽层断裂或端接不良','shield',3,4],['防止导体对屏蔽层短路','内绝缘损伤造成芯线与屏蔽层短路','short',2,4],['在规定弯曲半径下布置','弯曲半径不足导致导体或绝缘疲劳','routing',3,3],['防止与车身锐边、运动件摩擦','线缆磨损导致高压对车身短路','routing',3,3],['承受冷却液、油液和清洗剂','化学介质导致护套溶胀或开裂','corrosion',3,5],['在高温环境下保持绝缘寿命','热老化导致绝缘强度下降','thermal',3,5],['保持压接区域密封','液体沿导体进入高压连接器','sealing',3,5],['保持线缆布置和固定间距','固定点缺失导致线缆振动磨损','routing',3,3],['承受短路电流热效应','短路期间导体或绝缘过热损坏','thermal',2,4]]],
    ['高压线束','互锁低压线缆','hvil',[
      ['防止HVIL两根导线相互短接','线间短路导致系统误判高压连接正常','short',3,4],['防止HVIL导线对地短路','对地短路导致状态误判','short',3,3],['防止HVIL导线对电源短路','对电源短路损坏检测电路','short',2,3],['在振动条件下保持回路稳定','导体或压接点间歇开路','vibration',4,5],['保持规定回路电阻','导体或端子高阻导致HVIL误报警','contact',4,4],['维持HVIL端子压接和保持力','端子压接不良或退针','crimp',3,3],['防止与高压回路产生干扰耦合','EMC干扰导致HVIL状态跳变','emc',3,5],['保持线路布置和机械防护','线缆磨损导致互锁回路失效','routing',3,3]]],
  ];
  const additionalL2=[];
  extraCatalog.forEach(([focus,child,parentKey,entries])=>entries.forEach(([childFunction,childFailure,controlKey,occurrence,detectability])=>{
    const parent=extraParents[parentKey],control=controlTemplates[controlKey];
    additionalL2.push(row(2,`L2-EX-${String(additionalL2.length+1).padStart(3,'0')}`,parent.D,focus,child,parent.G,`${focus}通过${child}实现：${childFunction}`,childFunction,parent.K,parent.J,`${childFailure}导致${parent.K}`,childFailure,`${child}：${control[0]}`,occurrence,`${child}：${control[1]}`,detectability,[focus,child,'子零件专项扩展']));
  }));
  const finalL2=[...consolidatedL2,...additionalL2];
  // 系统层只从实际二级条目中选择一个关键下级，不再直接输出独立三级行。
  synchronizeLowerLevel(system,finalL2);

  global.DFMEA_LIBRARY = { version:8, originalSystemCount:37, additionalAnalysisCount:additionalL2.length, source:'new_template.xlsx + 电气系统级别需求(1).xlsx', fields:['C','D','E','F','G','H','I','J','K','L','M','N','O','P'], rows:[...system,...finalL2] };
})(window);
