...
(function(root,factory){
  ...
  function connectorPosition(state,c,k,p){
    ...
    const same=c.connectors.filter(x=>x.side===k.side),ki=same.indexOf(k),pi=k.pins.indexOf(p),span=sideSpan(c,k.side),before=same.slice(0,ki).reduce((sum,item)=>sum+Math.max(0,item.pins.length-1)*PIN_GAP+CONNECTOR_GAP,0);let q;if(k.side==='left'||k.side==='right'){const usable=Math.max(0,c.h-50-span),x=k.side==='left'?c.x:c.x+c.w,y=snapCoordinate(state,c.y+40+usable/2+before+pi*PIN_GAP);q={x,y,side:k.side};}else{const usable=Math.max(0,c.w-40-span),x=snapCoordinate(state,c.x+20+usable/2+before+pi*PIN_GAP),y=k.side==='top'?c.y:c.y+c.h;q={x,y,side:k.side};}return adjustedPinPosition(q,c,p);
  }
  ...
  function stabilizePins(state,c,before,newPins=[]){
    ...
    newPins=newPins.filter(p=>c.connectors.some(k=>connectorIsPlaced(k)&&k.pins.includes(p)));const minimum=componentMinimumSize(c);c.w=Math.max(Number(c.w)||0,minimum.w);c.h=Math.max(Number(c.h)||0,minimum.h);
