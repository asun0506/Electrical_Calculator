...
(function(root,factory){
  ...
  function normalize(s){
    const d=demo();if(!s||!Array.isArray(s.components))return d;const meta={...d.meta,...(s.meta||{})};if(!sheetSizes[meta.sheetSize])meta.sheetSize='A3';meta.canvasZoom=clamp(Number(meta.canvasZoom)||1,.2,2);meta.showGrid=meta.showGrid!==false;meta.snapToGrid=meta.snapToGrid!==false;meta.gridSize=[5,10,20].includes(Number(meta.gridSize))?Number(meta.gridSize):10;meta.legendMoved=meta.legendMoved===true;meta.legendX=Number.isFinite(Number(meta.legendX))?Number(meta.legendX):null;meta.legendY=Number.isFinite(Number(meta.legendY))?Number(meta.legendY):null;meta.connectionSort=['manual','source','target'].includes(meta.connectionSort)?meta.connectionSort:'manual';if(meta.legendLabels&&typeof meta.legendLabels==='object'&&!Array.isArray(meta.legendLabels)){const labels={};Object.keys(lineTypes).forEach(key=>{if(Object.prototype.hasOwnProperty.call(meta.legendLabels,key))labels[key]=String(meta.legendLabels[key]??'').slice(0,200);});meta.legendLabels=labels;}else delete meta.legendLabels;
    if(typeof meta.legendNote==='string')meta.legendNote=meta.legendNote.replace(/[\r\n]+/g,' ').slice(0,200);else delete meta.legendNote;
    for(const [key,min,max] of [['componentNameFontSize',8,32],['pinNameFontSize',6,20]]){const value=Number(meta[key]);if(meta[key]!=null&&Number.isFinite(value))meta[key]=clamp(value,min,max);else delete meta[key];}
    for(const key of ['titleBlockX','titleBlockY','titleBlockW','titleBlockH','revisionBlockX','revisionBlockY','revisionBlockW','revisionBlockH']){const value=Number(meta[key]);if(meta[key]!=null&&Number.isFinite(value))meta[key]=clamp(value,0,10000);else delete meta[key];}
    ...
    connections.forEach(w=>{if(typeof w.colorOrderSwapped!=='boolean')delete w.colorOrderSwapped;if(!['start','end','both'].includes(w.gaugeLabelPosition))delete w.gaugeLabelPosition;});
